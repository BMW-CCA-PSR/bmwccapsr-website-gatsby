...
const API_BASE =
  process.env.SANITY_STUDIO_MOTORSPORTREG_API_BASE || 'https://api.motorsportreg.com'
const ORG_ID =
...
const MotorsportRegEventInput = (props) => {
  ...
  const operations = useDocumentOperation(publishedId || '', documentType || '')
        ...
        <Card padding={3} radius={2} border>
          <Flex gap={3} align="flex-start">
            ...
            <Stack space={2} flex={1}>
              ...
              <Box as="hr" style={{ border: 0, borderTop: '1px solid', margin: 0 }} />
              {formatEventDateRange(value?.start, value?.end) ? (
                <Text size={1} muted>
                  {formatEventDateRange(value?.start, value?.end)}
                </Text>
              ) : null}
              {[value?.venueName, value?.venueCity, value?.venueRegion].filter(Boolean)
                .length ? (
                <Text size={1} muted>
