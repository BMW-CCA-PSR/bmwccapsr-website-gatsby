#!/usr/bin/env node
/**
 * Start the WP Events Migration approval tool.
 * Opens http://127.0.0.1:<port>/events in the browser.
 *
 * Usage:
 *   node bin/startWpEventsMigrationApprovalTool.js [--port 8788] [--no-open]
 *
 * The tribe/events/v1 base URL defaults to the value of TRIBE_EVENTS_API_BASE
 * or the hardcoded EC2 instance URL.  Override via environment variable if needed.
 */
...
function main(argv = process.argv.slice(2)) {
  ...
  // Override the default port to 8788 so it doesn't clash with the posts tool on 8787.
  const args = ['--port', readFlag(argv, '--port', '8788'), ...argv.filter((a) => !a.startsWith('--port'))];

  // startServer opens '/' by default; we want '/events'.
  // Patch the open URL by overriding the callback convention via env var that
  // localReviewServer checks, or just let it open '/' and the user navigates to /events.
  // For convenience we print the events URL explicitly.
  const server = startServer(args, /* openPath= */ '/events');

  const port = readFlag(args, '--port', '8788');
  console.log(`Events approval UI: http://127.0.0.1:${port}/events`);
  return server;
}
