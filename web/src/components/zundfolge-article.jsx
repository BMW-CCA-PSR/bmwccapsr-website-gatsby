...
const ArticleNavCard = ({ post, label }) => {
  ...
  return (
    <Link to={getZundfolgeUrl(post.slug.current)} sx={{ textDecoration: "none" }}>
      <Card
        sx={{
          position: "relative",
          overflow: "hidden",
...
function ZundfolgeArticle(props) {
  ...
  const avatarUrls = authors
    ...
    .filter(Boolean)
    .map((image) =>
      imageUrlFor(image).width(avatarSize).height(avatarSize).fit("fill").auto("format").url()
    )
  return (
    <article>
      <ContentContainer sx={{
          ...
          <Flex sx={{py:"0.5rem", width: "100%", alignItems: "center", gap: "0.75rem"}}>
            {avatarUrls.length > 0 && (
              <Box
                sx={{
                  position: "relative",
                  width: `${avatarSize + (avatarUrls.length - 1) * avatarOffset}px`,
